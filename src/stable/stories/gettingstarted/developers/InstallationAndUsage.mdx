...
```js
<cod-service-button href="https://www.example.com" target="_blank" rel="noopener noreferrer">
...
are described on a components 'Docs' page.
...
```js
document.querySelector('cod-service-button').addEventListener('click', (event) => {
...
```js
document.querySelector('cod-service-button').addEventListener('custom-event', (event) => {
  console.log('Custom event dispatched!');
});
```

Custom events will be documents on the component's 'Docs' page.